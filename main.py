...
class BallApp(tk.Toplevel):
    ...
    def action(self):
        if self.start:
            ...
            if self.graf:
                self.canvas.delete( self.graf)
            if len(self.v_list) > 1:
                self.graf = self.canvas.create_line(self.v_list)
    ...
    def draw_background(self):
        ...
        self.canvas.create_rectangle(gr_x - 50, gr_y - 60, self.width, gr_y2 + 60, fill="white")
        self.canvas.create_rectangle(gr_x - 40, gr_y - 50, self.width, gr_y2 + 50)
        self.canvas.create_line(gr_x, gr_y - 50, gr_x2, gr_y2 + 50, width=2)
        self.canvas.create_line(gr_x - 40, gr_y3, self.width, gr_y3, width=2)
        for k in range(1, 10, 1):
            self.canvas.create_line((gr_x + k * 30, gr_y3 + 5), (gr_x + k * 30, gr_y3 - 5), width=3)
            self.canvas.create_text(gr_x + k * 30, gr_y3 + 20, text=str(k), font='Constantia 8')
        for k in range(-25, 26, 5):
            self.canvas.create_line((gr_x - 5, gr_y3 - k * 5), (gr_x + 5, gr_y3 - k * 5), width=3)
            self.canvas.create_text(gr_x - 20, gr_y3 - k * 5, text=str(k), font='Constantia 8')
        self.canvas.create_text(gr_x - 20, gr_y - 40, text='Vy(м/с)', font='Constantia 8')
        self.canvas.create_text(self.width - 20, gr_y3 - 20, text='t(с)', font='Constantia 8')
    ...
    def calc_max_aim_y(self, x):
        g = 9.8
        y = ((self.ball.Vmax * self.ball.Vmax) / (2 * g)) - ((g * x * x) / (2 * self.ball.Vmax * self.ball.Vmax))
        return y
